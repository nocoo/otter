...
`otter snapshot` 将快照保存到 `~/.config/otter/snapshots/` 目录。需要注意：

- **快照以明文 JSON 存储**，不加密。虽然凭据已脱敏，但快照仍包含配置文件内容、文件路径等信息
- 快照目录权限由操作系统用户权限保护，仅当前用户可访问
- 如需在共享设备上使用，建议定期清理旧快照（`~/.config/otter/snapshots/`）
- 未来版本可能支持加密存储
