// Native design study only. Derived from Lyre and Showtime (MIT).
// Copyright (c) 2026 Zheng Li; Copyright (c) 2026 nocoo.
// Exact sources, adaptations, and license: SOURCES.md.
import AppKit
import SwiftUI
...
struct OtterButtonStyle: ButtonStyle {
    enum Treatment { case standard, accent, destructive }
...
private struct OtterButtonBody: View {
    ...
    var body: some View {
        configuration.label
            ...
            .foregroundStyle(treatment == .standard ? Color.primary : treatment == .accent ? OtterTheme.onAccent : Color.white)
...
struct OtterMenu: View {
    ...
    var body: some View {
        Button(action: openMenu) {
            ...
            }.font(.system(size: 13, weight: .medium)).foregroundStyle(OtterTheme.ink)
                .padding(.horizontal, 11).frame(maxWidth: .infinity, alignment: .leading).frame(height: 32)
