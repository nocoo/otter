...
日期：2026-09-17。状态：3.0.0 本地实现与验收完成，尚未发布。评估基线：`main` / `2bd5871`，原产品版本 `2.1.0`。用户已确认本轮升级为 `3.0.0`，三个阶段均纳入本轮。
...
本轮已将 9 处版本来源统一为 `3.0.0`，完成本地实现、验证和构建，尚未发布。现有 release 脚本要求目标版本高于当前版本，不能对已经提升为 3.0.0 的工作树再次运行 `release -- 3.0.0`；正式发布时需识别已完成的版本准备，避免重复增版。后续版本仍通过既有 release 流程同步版本与产物。发行验证复用仓库现有核心、CLI/API、Web 和原生测试入口，并增加上述灾后取回与兼容场景；Mac 签名/公证和支持平台验收按既有发行边界独立完成。
...
- 安装包尚无 Apple 发行签名/公证，Intel 与 macOS 15 实机、完整 VoiceOver 和性能验收仍属独立发行检查。本轮不执行远程迁移、部署、推送、npm 发布或 GitHub Release。
