...
- **Single Cloudflare Worker hosts both `/api/*` and SPA assets**: `packages/worker` mounts the new D1-binding routes at `/api/*` (CF Access JWT + Bearer token), keeps the legacy `/v1/*` + `/health` + `/ingest/*` for any remaining HTTP-D1 callers, and serves SPA fallback via `[assets] directory = "../web/dist"` with `not_found_handling = "single-page-application"` and `run_worker_first = ["/api/*", "/v1/*", "/health", "/ingest/*"]`. Build chain: `bun run build` → wrangler picks up the dist.
