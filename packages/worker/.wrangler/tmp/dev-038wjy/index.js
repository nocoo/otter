...
// ../../node_modules/.bun/hono@4.12.8/node_modules/hono/dist/utils/color.js
...
// ../../node_modules/.bun/hono@4.12.8/node_modules/hono/dist/middleware/logger/index.js
...
async function apiKeyMiddleware(c, next) {
...
__name(apiKeyMiddleware, "apiKeyMiddleware");

// src/middleware/env-guard.ts
async function envGuardMiddleware(c, next) {
...
__name(envGuardMiddleware, "envGuardMiddleware");
...
function iconKey(hash, prefix) {
  return `${prefix}/${hash}.png`;
}
__name(iconKey, "iconKey");
async function putIcon(bucket, key, data) {
...
__name(putIcon, "putIcon");

// src/services/webhook.ts
function validateWebhookToken(db, token) {
  return db.prepare(`SELECT id, user_id, token, is_active FROM webhooks WHERE token = ?1`).bind(token).first();
}
__name(validateWebhookToken, "validateWebhookToken");

// src/routes/icons.ts
var iconsRoutes = new Hono2();
var ICON_HASH_PATTERN = /^[a-f0-9]{12}$/;
var MAX_ICONS_PER_REQUEST = 500;
var MAX_ICON_BASE64_SIZE = 15e4;
...
iconsRoutes.post("/:token/icons", async (c) => {
  ...
  const webhook = await validateWebhookToken(c.env.DB, token);
  if (!webhook) {
...
async function putSnapshot(bucket, key, jsonString) {
...
__name(putSnapshot, "putSnapshot");
async function getSnapshot(bucket, key) {
...
async function deleteSnapshot(bucket, key) {
...
function isValidSnapshot(data) {
  if (typeof data !== "object" || data === null) return false;
  const obj = data;
  return obj.version === 1 && typeof obj.id === "string" && typeof obj.createdAt === "string" && typeof obj.machine === "object" && obj.machine !== null && Array.isArray(obj.collectors);
}
__name(isValidSnapshot, "isValidSnapshot");
function extractMetadata(snapshot) {
...
__name(decompressBody, "decompressBody");
...
__name(toSnapshotResponse, "toSnapshotResponse");
app.get("/", async (c) => {
  ...
  const [rowsResult, countResult] = await Promise.all([
    ...
    db.prepare("SELECT COUNT(*) as total FROM snapshots WHERE user_id = ?1").bind(userId).first()
  ]);
...
app.delete("/:id", async (c) => {
  ...
  const db = c.env.DB;
  const bucket = c.env.SNAPSHOTS;
  const row = await db.prepare("SELECT id FROM snapshots WHERE id = ?1 AND user_id = ?2").bind(snapshotId, userId).first();
  if (!row) {
    return c.json({ error: "Snapshot not found" }, 404);
  }
  const key = snapshotKey(userId, snapshotId);
...
__name(toWebhookResponse, "toWebhookResponse");
app2.get("/", async (c) => {
  const userId = c.get("userId");
  const db = c.env.DB;
...
app2.post("/", async (c) => {
  ...
  ).bind(id, userId, token, label, now).run();
  return c.json(
    {
      webhook: {
        id,
...
app2.patch("/:id", async (c) => {
  ...
  const webhookId = c.req.param("id");
  const db = c.env.DB;
  const existing = await db.prepare("SELECT id, user_id FROM webhooks WHERE id = ?1").bind(webhookId).first();
  if (!existing) {
    return c.json({ error: "Webhook not found" }, 404);
  }
  ...
  await db.prepare(`UPDATE webhooks SET ${updates.join(", ")} WHERE id = ?${paramIndex}`).bind(...values).run();
...
// src/index.ts
var app3 = new Hono2();
app3.use("*", envGuardMiddleware);
app3.use("*", logger());
app3.route("/health", healthRoutes);
app3.route("/ingest", ingestRoutes);
app3.route("/ingest", iconsRoutes);
app3.use("/v1/*", apiKeyMiddleware);
app3.route("/v1/snapshots", app);
app3.route("/v1/webhooks", app2);
...
// .wrangler/tmp/bundle-UhFyX2/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UhFyX2/middleware-loader.entry.ts
