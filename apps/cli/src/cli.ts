...
const workspaceArgs = {
  ...
  format: { type: "string", description: "Structured output: json or versioned ndjson events" },
  "config-dir": {
    type: "string",
    description: "Explicit CLI config directory (or OTTER_CONFIG_DIR)",
  },
  "output-dir": { type: "string", description: "Absolute local snapshot directory" },
  "api-url": { type: "string", description: "Explicit API origin; HTTPS or loopback HTTP" },
  "job-id": { type: "string", description: "Caller-supplied ID for structured task events" },
} as const;

const otterConfigDir = workspaceConfigDirectory();
...
const scanCommand = defineCommand({
  ...
  args: {
    ...
    "scan-root": {
      type: "string",
      description: "Explicit home fixture; requires file-only --collectors",
    },
    collectors: { type: "string", description: "Comma-separated collector IDs" },
  },
  async run({ args }) {
...
const backupCommand = defineCommand({
  ...
  args: {
    ...
    "snapshot-sha256": {
      type: "string",
      description: "Refuse upload if the reviewed snapshot digest changed",
    },
    dev: {
      type: "boolean",
      description: "Use config.dev.json; API address remains explicitly configured",
      default: false,
    },
    slim: {
      type: "boolean",
...
const configCommand = defineCommand({
  ...
  args: {
    ...
    action: {
      type: "positional",
      description: "Action: get, set, show; use status --json for redacted native status",
      required: false,
    },
    key: {
      type: "positional",
...
export const main = defineCommand({
  ...
  subCommands: {
    capabilities: defineCommand({
      meta: {
        name: "capabilities",
        description: "Versioned native workspace protocol and resolved paths (JSON)",
      },
      args: { json: { type: "boolean", default: true } },
      async run() {
        await runWorkspaceCommand(["capabilities", "--json"], CLI_VERSION);
      },
    }),
    login: loginCommand,
    scan: scanCommand,
