...
function diffLists(
  ...
  const identity = (item: (typeof oldLists)[number]) => `${item.meta?.source ?? ""}\0${item.name}`;
  const oldMap = new Map(oldLists.map((l) => [identity(l), l]));
  const newMap = new Map(newLists.map((l) => [identity(l), l]));
  const entries: DiffEntry[] = [];

  for (const [key, item] of newMap) {
    const old = oldMap.get(key);
    if (!old) {
      entries.push({ type: "added", label: item.name });
    } else if (
      old.version !== item.version ||
      JSON.stringify(Object.entries(old.meta ?? {}).sort()) !==
        JSON.stringify(Object.entries(item.meta ?? {}).sort())
    ) {
      entries.push({ type: "changed", label: item.name });
    }
  }

  for (const [key, item] of oldMap) {
    if (!newMap.has(key)) {
      entries.push({ type: "removed", label: item.name });
    }
  }
...
export function formatSnapshotDiff(diff: SnapshotDiffResult): string {
  ...
  for (const c of diff.collectors) {
    ...
    for (const l of c.lists) {
      ...
      const color: TreeChild["color"] =
        l.type === "added" ? "green" : l.type === "removed" ? "red" : "yellow";
