...
import { skillDirectories } from "./skill-directories.js";
...
export class OpenCodeConfigCollector extends BaseCollector {
  ...
  private async collectSkillNames(
    ...
    const entries = await skillDirectories(dirPath, result.errors, false, false);
    for (const entry of entries) {
      const meta: Record<string, string> = { source };
      const location = `file://${join(dirPath, entry, "SKILL.md")}`;
      ...
      try {
        ...
        const skillMd = await readFile(join(dirPath, entry, "SKILL.md"), "utf-8");
