...
beforeAll(async () => {
  configDir = await mkdtemp(join(tmpdir(), "otter-l2-cli-"));
});
...
describe("L2 CLI binary", () => {
  ...
  it("config show in an isolated directory prints default config without crashing", () => {
  ...
  it("config set then config get round-trips a value through OTTER_CONFIG_DIR", () => {
