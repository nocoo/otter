...
for OTTER_ARCH in arm64 x64; do
  ...
  if [ ! -x "$OTTER_HELPER" ] || [ "$(cat "$OTTER_HELPER.sha256" 2>/dev/null || true)" != "$OTTER_FINGERPRINT" ]; then
    bun build "$OTTER_REPO/apps/cli/src/bin.ts" --compile --target="bun-darwin-$OTTER_ARCH" --outfile "$OTTER_HELPER" >&2
    printf '%s' "$OTTER_FINGERPRINT" > "$OTTER_HELPER.sha256"
  fi
  if [ "$OTTER_CACHE" != "$OTTER_OUTPUT" ]; then
    cp "$OTTER_HELPER" "$OTTER_OUTPUT/otter-$OTTER_ARCH"
  fi
done
