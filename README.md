...
- **Homebrew 采集** — formulae、casks、taps 列表
- **应用采集** — `/Applications` 目录下所有 `.app` 列表 + 图标提取
...
│   │   └── src/
│   │       ├── cli.ts               #   citty 命令注册
│   │       ├── collectors/          #   5 个采集器 + BaseCollector 基类
...
| L3 API E2E | Webhook 端到端、健康检查 | pre-push |
| L4 BDD E2E | 完整备份流程验证 | 按需 |

```bash
bun run test              # L1 全部单元测试
bun run lint              # L2 类型检查
bun run test:e2e          # L3 + L4 端到端测试
```
...
| [采集器详解](docs/02-collectors.md) | BaseCollector API、5 个采集器、新增指南 |
